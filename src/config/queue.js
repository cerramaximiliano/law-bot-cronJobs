...
const addJobToQueue = async (trackingCode, userId, trackingType, _id) => {
  try {
    ...
    if (!tracking.isEnqueued) {
      logger.info(`Intentando añadir trabajo a la cola: ${trackingCode}, _id: ${_id}`);
      ...
      logger.info(`Trabajo añadido y flag isEnqueued actualizado para: ${trackingCode}`);
    } else {
      logger.info(`El trabajo ya está en la cola para: ${trackingCode}`);
    }
  } catch (error) {
    logger.error(`Error al intentar añadir trabajo a la cola: ${error.message}`);
  }
};
